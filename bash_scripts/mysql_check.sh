...
IPADDR=$(ifconfig enp0s8|grep 'inet '|awk -F ' ' '{print $2}')
...
#定义报表的全局变量
report_DateTime="2012-11-22"    #日期 ok
report_Hostname="master"    #主机名 ok
report_OSRelease="Red Hat Enterprise Linux Server release 7.5"    #发行版本 ok
report_Kernel="3.10.0"    #内核 ok
report_Language="UTF-8"    #语言/编码 ok
report_LastReboot="Nov 22 10:27"    #最近启动时间 ok
report_Uptime="0"    #运行时间（天） ok
report_CPUs="1"    #CPU数量 ok
report_CPUType="Intel(R) Core(TM) i5-6200U CPU @ 2.30GHz"    #CPU类型 ok
report_Arch=""    #CPU架构 ok
report_MemTotal="1999"    #内存总容量(MB) ok
report_MemFree="875"    #内存剩余(MB) ok
